...
const projects = [
  {
    ...
    description:
      "Developed an ecommerce store with admin panel and jwt authentication using MERN stack",
    links: {
  ...
  {
    img: { chat, chatProfile, gemini, chatLogin },
    title: "Real time chat app",
    description:
      "Developed an real time chat app with socket.io and jwt authentication using MERN stack",
    links: {
...
const Portfolio = () => {
  ...
  return (
    <div
      className="flex flex-col items-center justify-center w-full my-2 md:my-10"
      id="projects"
    >
      <div className="flex flex-col w-full items-center justify-center">
        <h2 className="text-5xl font-bold text-gradient text-center font-sans mb-0 md:mb-2">
          Projects
        </h2>
        <div className="flex justify-between w-[90%] md:w-[64%] ite">
          <div className="flex flex-col">
            <h3 className="text-2xl font-bold text-white text-start font-sans">
              {currentProject.title}
            </h3>
            <h5 className="text-sm font-semibold text-gray-400 text-start font-sans">
              {currentProject.description}
            </h5>
          </div>
          <div className="flex gap-2 flex-col md:flex-row">
            <motion.button
              className="text-purple-500 font-semibold p-2 w-24 md:w-28 flex items-center gap-2 border-2 border-purple-500 rounded-full"
            ...
            <motion.button
              className="text-purple-500 w-24 md:w-28 p-2 font-semibold flex items-center justify-center gap-2 border-2 border-purple-500 rounded-full"
              whileHover={{ scale: 1.1 }}
              onClick={() => window.open(currentProject.links.site, "_blank")}
              disabled={currentProject.links.site === ""}
            >
        ...
        >
          <div className="relative w-full h-full overflow-hidden rounded-2xl">
            ...
            <motion.button
              ...
              className="absolute top-[47%] border border-purple-800 right-1 text-3xl  bg-purple-700 p-2 rounded-full text-white "
              onClick={nextProject}
            >
            ...
            <motion.button
              whileHover={{ scale: 1.2 }}
              className="absolute top-[47%] border border-purple-800 left-1 text-3xl  bg-purple-700 p-2 rounded-full text-white "
              onClick={prevProject}
            >
            ...
            <div className="relative flex items-center justify-center">
              {projectImages.map((img, index) => (
                <motion.button
                  key={index}
                  whileHover={{ scale: 1.2 }}
                  className={`mt-2 mx-1 border-2 border-purple-800 text-3xl p-1 rounded-full text-white ${
                    index === currentImageIndex ? "bg-purple-700" : ""
                  }`}
                  onClick={() => {
